...
namespace TraceOptimizer.Geometry
{
    public struct Cuboid : IEquatable<Cuboid>
    {
        ...
        public IEnumerable<Cuboid> YPlanesFromBottomToTop()
        {
            for (var y = MinPoint.Y; y <= MaxPoint.Y; y++)
                yield return Cuboid.FromPoints(
                    new Point3D(MinPoint.X, y, MinPoint.Z),
                    new Point3D(MaxPoint.X, y, MaxPoint.Z)
                );
        }
        ...
        public override string ToString() =>
            $"Cuboid(min: {MinPoint}, max: {MaxPoint})";
    }
}